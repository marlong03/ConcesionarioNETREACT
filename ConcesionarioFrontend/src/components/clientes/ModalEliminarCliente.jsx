...
function ModalEliminarCliente({
  ...
  const enviarEliminarCliente = async () => {
      ...
      <Box className="my-0" p={0} position="relative">
        ...
        <Box p={3} pt={5}>
          ...
          <Typography variant="h6">
            ...
            <strong>{clienteSeleccionado?.nombre || 'Desconocido'}</strong>?
          </Typography>
          ...
          <Box mt={3} display="flex" justifyContent="flex-end">
            <Button
              onClick={enviarEliminarCliente}
              variant="outlined"
              color="error"
              disabled={cargando}
            >
              {cargando ? 'Eliminando...' : 'Eliminar Cliente'}
            </Button>
          </Box>
        </Box>
      </Box>
